...
class WisconetStation:
    ...
    def get_field_names(self, filter: str | None = None) -> list[str]:
        ...
        if filter is None:
            ...
            return [
                ...
                if filter in field.standard_name
            ]
    ...
    def fetch_data(
        ...
        df = bulk_measures_to_df(
            bulk_measures_data,
            tz=self.station.station_timezone,
            station_id=self.station.station_id,
        )
        if df is None:
            return None
    ...
    def fetch_all_available_data(
        self, fields: list[str] | None = None, timeout: float = 60.0
    ) -> pd.DataFrame:
        """
    ...
    def __str__(self):
        return (
            f"WisconetStation: {self.station.station_name} ({self.station.station_id})"
        )
...
class Wisconet:
    ...
    def __init__(self):
        self.stations: list[WisconetStation] = [
            WisconetStation(s)
            for s in all_stations()
            if s.station_id not in NO_DATA_STATION_IDS
        ]
    ...
    def nearest_stations(
        self, lat: float, lon: float, range: float | None = None, n: int | None = 3
    ) -> list[tuple[WisconetStation, float]]:
        """
        Get the nearest station to a given latitude and longitude.
        :param lat: latitude
        :param lon: longitude
        ...
        nearest_stations = []
        for station in self.stations:
            nearest_stations.append((station, station.distance_to_point(lat, lon)))
    ...
    def get_station(self, station_id: str | int) -> WisconetStation | None:
        """
    ...
    def get_data(
        ...
        """
        stations = [
            self.get_station(station_id=station_id) for station_id in station_ids
        ]
        return fetch_data_multiple_stations(
            [x.station for x in stations if x], start_time, end_time, fields, limits
        )
