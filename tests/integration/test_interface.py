...
def test_wisconet_get_data(station_ids, start_time, end_time, fields):
    w = Wisconet()
    df = w.get_data(
        station_ids=station_ids, start_time=start_time, end_time=end_time, fields=fields
    )
    real_station_ids = [
        x.station.station_id
        for x in [w.get_station(station_id=station_id) for station_id in station_ids]
        if x
    ]
    station_count = len(real_station_ids)
    days = (pd.Timestamp(end_time) - pd.Timestamp(start_time)).days
    station_tzs = [
        x.station.station_timezone
        for x in [w.get_station(station_id=station_id) for station_id in station_ids]
        if x
    ]
    assert df is not None, "Expected a DataFrame"
    assert not df.empty, "Expected a non-empty DataFrame"
    assert len(df) == station_count * days * 24, (
        "Expected DataFrame to have the correct number of rows"
    )
    assert df.index.name == "collection_time", "Expected collection_time index"
    assert str(df.index.tz) == station_tzs[0], (
        "Expected returned df index collection_time to be in station timezone"
    )
    assert "station_id" in df.columns, (
        "Expected returned df to have a 'station_id' column"
    )
    assert "value" in df.columns, "Expected returned df to have a 'value' column"
    assert set(df.station_id.unique().tolist()) == set(real_station_ids), (
        "Expected station_id column to contain each real requested station"
    )
...
def test_wisconet_get_nearest_station(lat, lon, expected_station_name):
    ...
    assert station.station.station_name == expected_station_name, (
        "Expected Verona station"
    )
...
def test_wisconet_get_nearest_stations_top_3():
    ...
    assert all(isinstance(station, WisconetStation) for station, _ in stations), (
        "Expected all elements to be WisconetStation objects"
    )
