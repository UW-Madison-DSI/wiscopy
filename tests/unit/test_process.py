...
def test_bulk_measures_to_df():
    ...
    assert isinstance(df, pd.DataFrame), (
        "bulk_measures_to_df did not return a pd.DataFrame"
    )
    assert not df.empty, "bulk_measures_to_df returned an empty DataFrame"
    assert df["standard_name"].iloc[0] == "60min_dew_point_f_avg", (
        "bulk_measures_to_df did not return the expected field value"
    )
    assert "value" in df.columns, (
        "bulk_measures_to_df did not return the expected value column"
    )
