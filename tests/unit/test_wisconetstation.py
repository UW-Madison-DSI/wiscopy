...
def test_wisconetstation_distance_to_point(station_name, lat, lon, expected_distance):
    stations = all_stations()
    specific_station = [
        s for s in stations if s.station_name.lower() == station_name.lower()
    ][0]
    s = WisconetStation(specific_station)
    distance = s.distance_to_point(lat, lon)
    assert isinstance(distance, float), "Expected distance to be a float"
    assert distance >= 0, "Expected distance to be non-negative"
    assert within(distance, expected_distance, 50), (
        "Expected delta distance to be less then 50"
    )
